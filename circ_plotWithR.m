function a = circ_plotWithR(alpha, R, format, formats, colorMap, minmax, varargin)
%
% r = circ_plot(alpha, ...)
...
%     hold on
%     zz = .8*exp(1i*linspace(0, 2*pi, 101));
%     plot(real(zz), imag(zz), 'k--', [-2 2], [0 0], 'k:', [0 0], [-2 2], 'k:');
    zz = .5*exp(1i*linspace(0, 2*pi, 101));
    plot(real(zz), imag(zz), 'k-', [-.5 .5], [0 0], 'k:', [0 0], [-.5 .5], 'k:');
    hold on
%     zz = .4*exp(1i*linspace(0, 2*pi, 101));
%     plot(real(zz), imag(zz), 'k--', [-.7 .7], [0 0], 'k:', [0 0], [-.7 .7], 'k:');
    zz = .25*exp(1i*linspace(0, 2*pi, 101));
    plot(real(zz), imag(zz), 'k--', [-.5 .5], [0 0], 'k:', [0 0], [-.5 .5], 'k:');
    ...
    hold on;
    zm = r*exp(1i*phi);   
    scatter(real(z), imag(z),1000,[0,0,0],'filled')
    clim(minmax)
    f = plot(real(z), imag(z), formats,'LineWidth',6, 'MarkerSize',50,'color',[0,0,0]);
    f = f.Color;
    colormap(colorMap);
    cmap = colormap;
    cmin = min(minmax);
    cmax = max(minmax);
    c_normalized = r;
    line_color = interp1(linspace(cmin, cmax, size(cmap, 1)), cmap, c_normalized);
    line_color = max(line_color, 0); 
    plot([0 real(zm)], [0, imag(zm)],varargin{2:end-2},'Color',f)
